...
select khach_hang.ho_ten, hop_dong.ma_khach_hang , count(*) as kh_booking
from hop_dong 
join khach_hang  on hop_dong.ma_khach_hang = khach_hang.ma_khach_hang
 left join loai_khach  on loai_khach.ma_loai_khach = khach_hang.Ma_loai_khach
where loai_khach.ma_loai_khach in(select ma_loai_khach from loai_khach where ten_loai_khach = 'Diamond')
group by ma_khach_hang
order by kh_booking asc;
...
sum(dich_vu.chi_phi_thue + isnull(hop_dong_chi_tiet.so_luong * dich_vu.chi_phi_thue)) as tong_tien
...
select
 ...
 join hop_dong on hop_dong.ma_dich_vu = loai_dich_vu.ma_loai_dich_vu
 where dich_vu.ma_loai_dich_vu not in(
 ...
 where year(hop_dong.ngay_lam_hop_dong) = 2020 and year(hop_dong.ngay_lam_hop_dong) not in (2021);
...
select month (hop_dong.ngay_lam_hop_dong) as doanh_thu_thang, count(distinct khach_hang.ma_khach_hang) as sl_kh
from hop_dong 
join khach_hang on khach_hang.ma_khach_hang = hop_dong.ma_khach_hang
where year(hop_dong.ngay_lam_hop_dong) = 2021
group by doanh_thu_thang
order by doanh_thu_thang;
...
sum(dich_vu_di_kem.ma_dich_vu_di_kem) as sl_dv_dk
from hop_dong
...
select 
dich_vu_di_kem.ma_dich_vu_di_kem,loai_khach.ten_loai_khach
from dich_vu_di_kem
...
where month(hop_dong.ngay_lam_hop_dong)in (10,11,12) and year (hop_dong.ngay_lam_hop_dong) = 2020 and hop_dong.ma_hop_dong
not in (select ma_hop_dong from hop_dong where month(hop_dong.ngay_lam_hop_dong) in (1,2,3) and year (hop_dong.ngay_lam_hop_dong) = 2021)
...
join hop_dong_chi_tiet hdct on hdct.ma_dich_vu_di_kem = hdct.ma_dich_vu_di_kem
